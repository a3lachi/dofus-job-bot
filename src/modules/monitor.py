...
'''
    Import module by its path regardless of Os
    VERY UGLY FUNCTION THAT WAS ONCE 2 FUNCTIONS EMBEDDING BOTH IF EL

    USE CASE : 

    config = import_module('../config')
    performance = import_module('../utils/performance')

    performance.scope_time(False)   // call with module.method / module.variable
    print(56,scope_time(True))      // call method / variable
    print(config.P_MOUSE_MIN_HEIGHT)
    print(P_MOUSE_MIN_HEIGHT)

'''
def import_module(module_path):
    curr_os = platform.system()
    add_path = '/'.join(module_path.split('/')[:-1]) if curr_os == 'Darwin' else '//'.join(module_path.split('//')[:-1]) if curr_os == 'Darwin' else ""
    add_path = add_path.split('../')
    curr_os = platform.system()
    config_path = '/'.join(sys.argv[0].split('/')[:-len(add_path)])+'/'+add_path[-1] if (curr_os == 'Darwin') else '/'.join(sys.argv[0].split('//')[:-len(add_path)])+'//'+add_path[-1] if (curr_os == 'Windows') else ""
    sys.path.append(os.path.abspath(config_path))
    module_to_import = module_path.split('/')[-1] if curr_os == 'Darwin' else module_path.split('//')[-1] if curr_os == 'Darwin' else ""
...
performance.scope_time(False)
print(56,performance.scope_time(True))

print(config.P_MOUSE_MIN_HEIGHT,performance.scope_time(True))

aa = '999'
print('This printing took this amount of time',aa,performance.scope_time(True))



aa = '9999999999999999999999999999999999999999'
print(aa,performance.scope_time(True))


print(56)
print(scope_time(True))
# import pyautogui
